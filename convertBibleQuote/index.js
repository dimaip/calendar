...
const parseVerseContent = (verse) => verse
	.replace(/<\/p>/g, '')
	.replace(/\s+\d{1,6}/g, '')
	.replace(/\s+/g, ' ');

const parseChapterContent = (chapterContent, verseSign) => chapterContent.split(verseSign).slice(1).filter(i => i && i.trim()).map(vvv => {
	try {
...
const parseBookContent = (bookContent, translationKey, bookKey, chapterSign, verseSign) => bookContent.split(new RegExp(chapterSign)).slice(1).filter(i => i).map(vv => {
	try {
		const [chapterNumber, chapterContent] = vv.split(/(\d{1,3}?)<[^>]+?>/).slice(1);
		if (!chapterContent) {
			throw new Error(`chapterContent not parsed in (${vv}) - ${translationKey}/${bookKey}/${chapterNumber}`);
		}
