...
const runLint = () => {
    const result = spawnSync(
        process.execPath,
        [path.join(repositoryRoot, 'node_modules/eslint/bin/eslint.js'), 'app', '--format', 'json'],
        {
            cwd: repositoryRoot,
            encoding: 'utf8',
    ...
    for (const report of reports) {
        if (report.fatalErrorCount > 0) {
            throw new Error(`ESLint could not parse ${report.filePath}; fatal errors cannot be baselined.`);
        }
...
const runTypeScript = () => {
    ...
    for (const line of `${result.stdout}\n${result.stderr}`.split(/\r?\n/)) {
        ...
        if (projectDiagnostic) {
            throw new Error(`TypeScript project/configuration failure: ${line}`);
        }
    ...
    if (result.status !== 0 && diagnostics === 0) {
        throw new Error(
            result.stderr || result.stdout || `TypeScript failed without readable diagnostics (${result.status}).`
        );
    }
