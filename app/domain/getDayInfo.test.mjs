...
test.todo('MOD-04: calculates Easter correctly after the Gregorian/Julian offset changes in 2100', () => {
...
test('characterizes the current week-of-sorrow boundaries, including the November 7 noon limitation', () => {
    ...
    // Current behavior; the correction is tracked separately in MOD-04.
    assert.equal(isNedelaSkorbi(atLocalNoon(2026, 11, 7)), false);
    assert.equal(isNedelaSkorbi(atLocalNoon(2026, 11, 8)), false);
});

test.todo('MOD-04: the November 7 week-of-sorrow result is independent of the Date time component', () => {
    assert.equal(isNedelaSkorbi(atLocalNoon(2026, 11, 7)), true);
});

test.todo('MOD-04: memoized day-info keys distinguish February double-digit dates from December single-digit dates', () => {
